...
import { SelectedIdentityType, ProfileType } from '../../types/ProfileType';
...
export default function PaymentDialog({
    ...
    if (confirmedCombined && txHashCombined && (!crossChainMode || crossChainPaymentTxHash)) {
      ...
      if (profile) {
        ...
        if (isNativeFlow && !paymentWallet.deployed) {
          ...
          await updateWallet(selectedFlow.uuid, paymentWallet);
        }
      }
  ...
  const submitGlideTransaction = async () => {
    if (address?.toLowerCase() !== selectedFlow.signer.toLowerCase()) {
  ...
  async function submitTransaction(tx?: ViemTransaction) {
    if (paymentType !== 'wallet' && address?.toLowerCase() !== selectedFlow.signer.toLowerCase()) {
      setOpenConnectSignerDrawer(true);
      return;
    }
  ...
  useMemo(async () => {
    if (
      isNativeFlow &&
      address?.toLowerCase() === selectedFlow.signer.toLowerCase() &&
      paymentToken?.chainId
    ) {
      await switchChainAsync({ chainId: paymentToken.chainId });
    }
  }, [isNativeFlow, address, selectedFlow, paymentToken?.chainId]);
          ...
          <ResponsiveDialog
            ...
            <Stack alignItems="flex-start" spacing={2}>
              <Typography variant="caption" color={grey[prefersDarkMode ? 400 : 700]}>
                Selected payment flow `<b>{selectedFlow.title}`</b> signer is not connected! Please,
