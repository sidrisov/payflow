...
if (!AA_FACTORY_ADDRESS) {
  throw new Error(
    'Please set AA_FACTORY_ADDRESS in the environment variables.'
  );
}
...
if (!MULTISIG_OWNER_ADDRESS_1) {
  throw new Error(
    'Please set MULTISIG_OWNER_ADDRESS_1 in the environment variables.'
  );
}
...
if (!MULTISIG_OWNER_ADDRESS_2) {
  throw new Error(
    'Please set MULTISIG_OWNER_ADDRESS_2 in the environment variables.'
  );
}
...
export default async function (hre: HardhatRuntimeEnvironment) {
  ...
  const multisigAddress = utils.create2Address(
    ...
    abiCoder.encode(
      ['address', 'address'],
      [MULTISIG_OWNER_ADDRESS_1, MULTISIG_OWNER_ADDRESS_2]
    )
  );
