...
export default function PaymentDialog({
  ...
  // Derived data
  const recipientCompatibleFlows = useMemo(() => {
    return profile?.flows?.filter(
      (flow) =>
  ...
  const { regularPaymentTx, glidePaymentTx } = useMemo(() => {
    ...
    if (crossChainMode) {
      const { paymentChainId, tokenId, amount } = getPaymentDetails();
  ...
  // Helper function for getting payment details
  function getPaymentDetails() {
    ...
    return { paymentChainId, tokenId, amount };
  }
  ...
  }, [toAddress, paymentAmount, paymentAmountUSD]);
  ...
  // Handle cross-chain mode toggle
  const handleCrossChainModeChange = (_: React.MouseEvent<HTMLElement>, newMode: string | null) => {
    if (newMode !== null) {
      setCrossChainMode(newMode === 'cross-chain');
    }
  };

  // Render success dialog or payment dialog
  ...
  // Helper function to render dialog content
        ...
        <Box ml={1} width="100%" display="flex" alignItems="start" justifyContent="space-between">
          ...
          <Box>
            ...
            <ToggleButtonGroup
              ...
              exclusive
              onChange={handleCrossChainModeChange}
              size="small">
  ...
  // Helper function to render cross-chain payment info
  function renderCrossChainPaymentInfo() {
    if (!crossChainMode) return null;
